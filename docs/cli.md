...
Results go to stdout as a JSON array, in the same shape the discover function returns:
...
With `--include-invalid`, the `error` of an invalid result is printed as its message. When nothing is found, the output is an empty array.
